...
for(k=1:8)
   sensorTally = sensorTally+wb_distance_sensor_get_value(ps(k));
end
...
while(sensorTally<tooClose)
    ...
    [xEnc,yEnc,phiEnc] = odometry2(xEnc,yEnc,newLeft-oldLeft, newRight-oldRight, phiEnc);
...
while(1)  %%arbitrary wallFollowing end point
  ...
  position = sprintf('lastSensorLeftBack: %d , sensorLeftBack: %d', lastSensorLeftBack, sensorLeftBack);
  disp(position);
  
  % control function
  pdControlFunction = distanceError*Kp + distanceDelta*Kd; 
  
  position = sprintf('distanceError: %d , distanceDelta: %d', distanceError, distanceDelta);
  disp(position);
  ...
  if(sensorFrontLeft < far && sensorFrontRight < far ...
    ...
    if ~isnan(pdControlFunction)
        vRight = vRight + pdControlFunction;
    end
    
    controlInfo = sprintf('Moving forward! Left Wheel: %d Right Wheel: %d... forward norm %d and pdFunc %d', vLeft, vRight, forwardNorm, pdControlFunction); 
    followFlag = following;
  ...
  % end of time-step; update variables
  oldLeft = newLeft;
  oldRight = newRight;
  TIMER = TIMER +1;
  lastSensorLeftBack = sensorLeftBack;
  
  if (abs(xStart - xEnc) < 0.1 && abs(yStart - yEnc) < 0.1 && TIMER > 500)
       disp('HOME')
       wb_differential_wheels_set_speed(0, 0);
       wb_robot_step(64);
       break
  ...
  debug = sprintf('distance translated x: %d. distance translated y: %d', abs(x-xStart), abs(y-yStart));
  disp(debug);
